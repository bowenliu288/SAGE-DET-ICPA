...
# ============================================================
# SparseTrack Windows fallback:
# pbcvt is an optional compiled GMC extension in the official
# repository. If it is unavailable, use identity motion
# compensation. Baseline and ICPA-SparseTrack must use the
# same setting for a fair paired comparison.
# ============================================================
try:
    from tracker import pbcvt
    PBCVT_AVAILABLE = True
except ImportError:
    pbcvt = None
    PBCVT_AVAILABLE = False
...
class SparseTracker(object):
    def __init__(self, args, frame_rate = 30):
        ...
        # ====================================================
        # ICPA-SparseTrack v1 parameters
        # ====================================================
        self.icpa_center_thresh = getattr(
            args,
            "icpa_center_thresh",
            0.70
        )

        self.icpa_small_area = getattr(
            args,
            "icpa_small_area",
            32 * 32
        )
        ...
        # Compatibility name:
        # this is an IoU-overlap degradation threshold,
        # not a physical motion threshold.
        self.icpa_motion_thresh = getattr(
            args,
            "icpa_motion_thresh",
            0.70
        )
    ...
    def DCM(
        ...
        if len(track_mask) != 0:
            ...
            for dm, tm in zip(det_mask, track_mask):
                ...
                # ================================================
                # Association inside one SparseTrack depth subset.
                #
                # High-score first DCM:
                #   ICPA = native SparseTrack IoU(+score fusion)
                #          + independent center branch.
                #
                # Low-score DCM:
                #   original SparseTrack IoU association unchanged.
                # ================================================
                if use_icpa:
                    (
                        matches,
                        u_track_,
                        u_det_
                    ) = parallel_iou_center_assignment_icpa_sparse(
                        track_,
                        det_,
                        match_thresh=thresh,
                        center_thresh=self.icpa_center_thresh,
                        small_area=self.icpa_small_area,
                        motion_thresh=self.icpa_motion_thresh,
                        fuse_score_flag=(
                            (not self.args.mot20)
                            and
                            is_fuse
    ...
    def update(self, output_results, curr_img = None):
        ...
        # GMC
        #
        # Official SparseTrack uses the separately compiled
        # pbcvt extension here. On Windows, if pbcvt is absent,
        # fall back to an identity transform (GMC disabled).
        #
        # IMPORTANT:
        # Use exactly the same setting for SparseTrack baseline
        # and ICPA-SparseTrack.
        # ====================================================
        if PBCVT_AVAILABLE and self.pre_img is not None:
            warp = pbcvt.GMC(
                curr_img,
                self.pre_img,
                self.down_scale
