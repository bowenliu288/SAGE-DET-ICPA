...
__all__ = "OBB", "Classify", "Detect", "PGLiteDetect", "Pose", "RTDETRDecoder", "Segment", "YOLOEDetect", "YOLOESegment", "v10Detect"
...
class PGLiteDetect(Detect):
    """Position-Gaussian Quality-aware Lightweight Detection Head for small UAV detection.

    This head keeps the original YOLO26 output format and loss path unchanged. It introduces a
    lightweight learned position prior, a quality-calibrated classification bias, and a P2-only
    residual box refinement branch. The design is intended to complement CBDDownLite and
    P5SemanticGate: the former preserves small-target details, the latter injects high-level
    semantic guidance into P2, and this head focuses on final-stage score calibration and fine
    localization while keeping reg_max=1 for low computational cost.
    """

    def __init__(self, nc: int = 80, reg_max=1, end2end=False, ch: tuple = ()):  # noqa: D417
        """Initialize PGQ-Lite detection head.

        Args:
            nc (int): Number of classes.
            reg_max (int): Regression bins. Keep reg_max=1 for the lightweight version.
            end2end (bool): Whether to use YOLO26 end-to-end mode.
            ch (tuple): Input channels of detection feature maps.
        """
        super().__init__(nc=nc, reg_max=reg_max, end2end=end2end, ch=ch)

        # Learnable gains are initialized small so the head starts close to the original Detect behavior.
        self.pgq_refine_gain = nn.Parameter(torch.tensor(0.10))
        self.pgq_quality_gain = nn.Parameter(torch.tensor(0.05))

        # Lightweight learned position prior and quality calibration branches for each detection scale.
        # These use 1x1 projections to avoid high FLOPs on the P2 feature map.
        self.pgq_prior = nn.ModuleList()
        self.pgq_quality = nn.ModuleList()

        # P2-only localization refinement. P3/P4 keep the original Detect regression output to control FLOPs.
        self.pgq_refine = nn.ModuleList()

        for i, c in enumerate(ch):
            hid = max(8, min(32, c // 4))

            self.pgq_prior.append(
                nn.Sequential(
            ...
            self.pgq_quality.append(
                nn.Sequential(
                    nn.Conv2d(c, hid, 1, bias=False),
                    nn.BatchNorm2d(hid),
                    nn.SiLU(inplace=True),
                    nn.Conv2d(hid, self.nc, 1),
                )
            )

            if i == 0:
                # P2 has the highest spatial resolution and is the main small-UAV detection scale.
                # The branch predicts a bounded residual correction for the 4 box-distance logits.
                self.pgq_refine.append(
                    nn.Sequential(
                ...
                self.pgq_refine.append(nn.Identity())

        self._init_pgq_branches()

    def _init_pgq_branches(self):
        """Initialize auxiliary branches so the model begins close to baseline Detect."""
        for prior in self.pgq_prior:
            nn.init.constant_(prior[-1].weight, 0.0)
            nn.init.constant_(prior[-1].bias, 0.0)
        for quality in self.pgq_quality:
            nn.init.constant_(quality[-1].weight, 0.0)
            nn.init.constant_(quality[-1].bias, 0.0)
        for refine in self.pgq_refine:
            if isinstance(refine, nn.Sequential):
                nn.init.constant_(refine[-1].weight, 0.0)
                nn.init.constant_(refine[-1].bias, 0.0)

    def forward_head(
        self, x: list[torch.Tensor], box_head: torch.nn.Module = None, cls_head: torch.nn.Module = None
    ) -> dict[str, torch.Tensor]:
        """Return refined box logits and quality-calibrated class logits.

        The tensor layout remains identical to Detect:
            boxes:  [B, 4 * reg_max, sum(H_i * W_i)]
            scores: [B, nc,          sum(H_i * W_i)]
        """
        if box_head is None or cls_head is None:  # for fused inference
            return dict()
        ...
        for i in range(self.nl):
            feat = x[i]
            box = box_head[i](feat)
            score = cls_head[i](feat)

            # Learned position prior. At initialization it is 0.5 everywhere; during training it becomes
            # a target-sensitive spatial prior similar in spirit to position-Gaussian guidance.
            prior = self.pgq_prior[i](feat).sigmoid()

            # Quality calibration: only changes the classification logits, keeping output dimensions unchanged.
            # This encourages high-confidence predictions to be spatially consistent with learned object regions.
            q_bias = torch.tanh(self.pgq_quality[i](feat)) * prior
            score = score + self.pgq_quality_gain * q_bias

            # P2-only residual localization refinement. It is bounded and prior-gated to avoid unstable offsets.
            if i == 0 and isinstance(self.pgq_refine[i], nn.Sequential):
                delta = torch.tanh(self.pgq_refine[i](feat)) * prior
                box = box + self.pgq_refine_gain * delta
    ...
    def fuse(self) -> None:
        """Remove the one2many head for inference optimization in end-to-end mode."""
        self.cv2 = self.cv3 = self.pgq_prior = self.pgq_quality = self.pgq_refine = None
...
class Detect_LSCSBD(Detect):
    """Lightweight Shared Convolutional Separate BN Detection Head for YOLO26."""

    def __init__(self, nc=80, hidc=256, reg_max=1, end2end=True, ch=()):
        """Initialize LSCSBD detection head for YOLO26.

        Args:
            nc (int): Number of classes.
            hidc (int): Hidden channel dimension of the shared detection head.
            reg_max (int): Regression bins. YOLO26 default is reg_max=1.
            end2end (bool): Whether to use YOLO26 end-to-end mode.
            ch (tuple): Input channels of detection feature maps.
        """
        super().__init__(nc=nc, reg_max=reg_max, end2end=end2end, ch=ch)
        ...
        # Project multi-scale feature maps to the same hidden channel dimension.
        self.stem = nn.ModuleList(nn.Sequential(Conv(x, hidc, 3)) for x in ch)

        # Shared convolution layers across detection scales.
        self.share_conv = nn.ModuleList(
            [
                nn.Conv2d(hidc, hidc, 3, 1, 1),
                nn.Conv2d(hidc, hidc, 1, 1, 0),
            ]
        )

        # Separate BN for each detection scale and each shared convolution layer.
        # separate_bn[i][j]:
        # i -> detection scale index, such as P3/P4/P5
        # j -> shared convolution index
        self.separate_bn = nn.ModuleList(
            nn.ModuleList(nn.BatchNorm2d(hidc) for _ in range(len(self.share_conv)))
            for _ in ch
        )

        self.act = nn.SiLU()

        # LSCSBD one-to-many detection heads.
        self.cv2 = nn.ModuleList(
            nn.Conv2d(hidc, 4 * self.reg_max, 1) for _ in ch
        )
        self.cv3 = nn.ModuleList(
            nn.Conv2d(hidc, self.nc, 1) for _ in ch
        )

        # YOLO26 end-to-end mode needs one-to-one detection heads.
        # Do not assign self.one2many or self.one2one directly,
        # because they are properties in YOLO26 Detect.
        if self.end2end:
    ...
    def forward_head(
        ...
        """Forward LSCSBD head while keeping YOLO26 Detect output format.

        Important:
            Do not modify x in-place, because YOLO26 end-to-end Detect
            calls forward_head twice for one-to-many and one-to-one branches.
        """
        ...
        # LSCSBD feature transformation without modifying original x.
        for i in range(self.nl):
            xi = self.stem[i](x[i])

            for j, conv in enumerate(self.share_conv):
                xi = conv(xi)
                xi = self.separate_bn[i][j](xi)
                xi = self.act(xi)
        ...
        boxes = torch.cat(
            [
                box_head[i](feats[i]).view(bs, 4 * self.reg_max, -1)
                for i in range(self.nl)
            ],
            dim=-1,
        )
        ...
        scores = torch.cat(
            [
                cls_head[i](feats[i]).view(bs, self.nc, -1)
                for i in range(self.nl)
            ],
            dim=-1,
        )
    ...
    def bias_init(self):
        """Initialize LSCSBD Detect biases."""
