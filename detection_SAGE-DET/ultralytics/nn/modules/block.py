...
class SpaceToDepth(nn.Module):
    """
    Internal Space-to-Depth operation used by DCDM.
...
class DCDM(nn.Module):
    """
    Detail-preserving Context-aware Downsampling Module (DCDM).

    YAML usage:
        - [-1, 1, DCDM, []]

    SpaceToDepth is used only inside DCDM.
    """
    ...
    def __init__(self, c1, c2=None):
        ...
        c_context = c2 // 4
        c_spatial = c2 // 2
        c_boundary = c2 - c_context - c_spatial

        self.context_branch = nn.Sequential(
            Conv(c1, c1, k=3, s=2, g=c1),
            Conv(c1, c_context, k=1, s=1),
        )

        self.s2d = SpaceToDepth()
        self.spatial_branch = Conv(4 * c1, c_spatial, k=1, s=1)

        self.boundary_conv = nn.Sequential(
            Conv(c1, c1, k=3, s=1, g=c1),
            Conv(c1, c_boundary, k=1, s=1),
        )

        self.boundary_gate = nn.Sequential(
            nn.Conv2d(c_boundary, c_boundary, kernel_size=1, stride=1, padding=0, bias=True),
            nn.Sigmoid(),
        )

    def forward(self, x):
        fc = self.context_branch(x)

        fs = self.spatial_branch(self.s2d(x))

        max_p = F.max_pool2d(x, kernel_size=2, stride=2)
        avg_p = F.avg_pool2d(x, kernel_size=2, stride=2)
        bd = torch.abs(max_p - avg_p)

        fb = self.boundary_conv(bd)
        fb = fb * self.boundary_gate(fb)

        y = torch.cat([fc, fs, fb], dim=1)
        return y



class P5SGM(nn.Module):
    """
    P5 Semantic Guidance Module (P5-SGM).

    Inputs:
        x[0]: P2 feature, high-resolution small-object feature
        x[1]: P5 feature, high-level semantic feature

    Output:
        Enhanced P2 feature
    """

    def __init__(self, c_p2, c_p5):
        super().__init__()
        self.proj = Conv(c_p5, c_p2, k=1, s=1, act=False)
        self.act = nn.Sigmoid()

        # Learnable residual scale.
        # Initialized as 0 to make the module start from identity mapping.
    ...
    def forward(self, x):
        ...
        gate = self.proj(p5)
        gate = nn.functional.interpolate(gate, size=p2.shape[2:], mode="nearest")
        gate = self.act(gate)

        out = p2 + self.alpha * p2 * gate
        return out